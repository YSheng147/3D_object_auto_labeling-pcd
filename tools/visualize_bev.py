...
def plot_boxes(ax, boxes_lidar, color=[0,0,1], 
    ...
    for idx, box in enumerate(box_pts_bev): 
        ...
        if (scores is not None) and show_score:
            #ax.text(box[0,0], box[0,1], f'{scores[idx]:0.4f}', c=[1,0,0], fontsize=fontsize+4)            
            ax.text(box[0,0], box[0,1], f'{box_pts[idx,0,2]:0.2f}~{box_pts[idx,4,2]:0.2f}', c=[1,0,0], fontsize=fontsize)                        
...
def main():
    ...
    fig.canvas.manager.window.geometry("1200x800+100+100")
    ax = plt.subplot(111)
    ...
    if args.ps_pkl is not None:
        ...
        plot_boxes(ax, ps_dict[ps_frame_ids[start_idx]]['gt_boxes'][combined_mask], 
                ...
                label='ps labels', color=[0,0.8,0], fontsize=14, linewidth=3,
                limit_range=limit_range, alpha=1, show_score=False if args.hide_score else True)
    ...
    class Index:
        ...
        def toggle_play(self, event):
            if not self.playing:
                self.timer.start()
                self.playing = True
                event.inaxes.set_label('Pause')  # 按鈕文字改成暫停
            else:
                self.timer.stop()
                self.playing = False
                event.inaxes.set_label('Play')   # 按鈕文字改成播放
            plt.draw()
    ...
    # 建立計時器，間隔時間（毫秒）可調整
    callback.timer = fig.canvas.new_timer(interval=200)  # 0.2秒一幀
    callback.timer.add_callback(callback.auto_step)
    ...
    axplay = fig.add_axes([0.87, 0.05, 0.05, 0.045])
    bplay = Button(axplay, 'Play')
    bplay.on_clicked(callback.toggle_play)
    ...
    #ax.set_xlim(-30, 30)
    #ax.set_ylim(0, 80)
    fig.subplots_adjust(left=0.03, bottom=0.1, right=0.99, top=0.9, wspace=0.2, hspace=0.2)
