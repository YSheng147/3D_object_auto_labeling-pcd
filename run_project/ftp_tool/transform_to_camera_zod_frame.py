...
from data_loader_zod_frame import SceneDataLoader # 假設你的 loader 存檔為 data_loader.py
...
# zod
VIEW_EYE_POSITION = np.array([0.0, 10.0, 5.0])
...
def update_geometries(vis, state, camera_mode="vehicle"):
    ...
    gt_boxes = frame_data['boxes_gt']
    yaw_pose = frame_data['yaw_pose'] 
    calib = frame_data['calib']
    Tr_lidar_to_cam = calib.get('extrinsic_lidar_to_camera', np.eye(4))

    print(f"幀: {frame_idx} | Pred: {pred_boxes.shape[0]} | GT: {gt_boxes.shape[0]}", end="\r")
    
    vis.clear_geometries() # 先清除舊的



    points_in_camera = transform_points(np.asarray(pcd_legacy.points), Tr_lidar_to_cam)
    pcd_camera = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(points_in_camera))
    
    # --- 修改開始：根據「深度 (距離)」上色 ---
    # Camera 座標系中，Z 軸是前方距離
    depth_values = points_in_camera[:, 2] 
    
    # 設定可視距離範圍 (例如 0 到 80 公尺)
    # 超過 80 公尺的點會變成純紅，近處為藍
    min_dist = 0.0
    max_dist = 80.0 
    
    norm_dist = np.clip((depth_values - min_dist) / (max_dist - min_dist), 0.0, 1.0)
    pcd_camera.colors = o3d.utility.Vector3dVector(cm.jet(norm_dist)[:, :3])
    # --- 修改結束 ---

    vis.add_geometry(pcd_camera) # 加入點雲

    # 處理與加入框 (這部分不受開關影響，總是顯示)
    corners_pred = transform_boxes_to_corners(pred_boxes, Tr_lidar_to_cam)
    corners_gt = transform_boxes_to_corners(gt_boxes, Tr_lidar_to_cam)
    
    lineset_pred = create_lineset_from_corners(corners_pred, color=[1.0, 0.0, 0.0]) 
    lineset_gt = create_lineset_from_corners(corners_gt, color=[0.0, 1.0, 0.0])   

    coordinate_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=1.0, origin=[0, 0, 0])

    vis.add_geometry(lineset_pred)
    vis.add_geometry(lineset_gt)
...
def render_video_sequence(state, output_filename, framerate):
    ...
    ffmpeg_command = [
        'ffmpeg', '-r', str(framerate), '-i', f'{temp_image_folder}/frame_%05d.png',
...
def draw_projection_on_image(image, points_2d, corners_2d, color_box=(0, 0, 255)):
    """ 為了向下相容，保留這個函式，但建議用下面的專用函式 """
    ...
    for point in points_2d[in_bounds].astype(np.int32):
        cv2.circle(image, tuple(point), radius=1, color=(0, 255, 0), thickness=-1)

    draw_projection_boxes(image, corners_2d, color_box)
    return image

def draw_projection_boxes(image, corners_2d, color=(0, 0, 255)):
    """ 專門畫框的函式，方便指定顏色 """
    if len(corners_2d) == 0: return
    edges = [(0, 1), (1, 2), (2, 3), (3, 0), (4, 5), (5, 6), 
             (6, 7), (7, 4), (0, 4), (1, 5), (2, 6), (3, 7)]
    for box_corners in corners_2d:
        pts = box_corners.astype(np.int32)
        for i, j in edges:
            cv2.line(image, tuple(pts[i]), tuple(pts[j]), color=color, thickness=2)

def render_2d_video_sequence(state, output_filename, framerate):
    """ 渲染 2D 影片 (修正 UnboundLocalError) """
    ...
    for i in range(state.total_frames):
        ...
        calib = frame_data['calib']
        K = calib.get('intrinsic', np.eye(3))
        Tr_lidar_to_cam = calib.get('extrinsic_lidar_to_camera', np.eye(4))
        ...
        # 1. 畫點 (如果沒有計算點雲，projected_points 就是空的，這裡會直接跳過，不會報錯)
        draw_projection_on_image(canvas, projected_points, [], color_box=(0,0,0))
        ...
        if corners_pred.shape[0] > 0:
            valid_mask = (corners_pred[:, :, 2] > 0).all(axis=1)
            valid_corners = corners_pred[valid_mask]
            if valid_corners.shape[0] > 0:
                proj_pts, _ = project_points_to_image(valid_corners.reshape(-1, 3), K)
                projected_corners_pred = proj_pts.reshape(-1, 8, 2)
                draw_projection_boxes(canvas, projected_corners_pred, color=(0, 0, 255)) 

        # 處理 GT 框 (綠色)
        corners_gt = transform_boxes_to_corners(gt_boxes, Tr_lidar_to_cam)
        if corners_gt.shape[0] > 0:
            valid_mask = (corners_gt[:, :, 2] > 0).all(axis=1)
            valid_corners = corners_gt[valid_mask]
            if valid_corners.shape[0] > 0:
                proj_pts, _ = project_points_to_image(valid_corners.reshape(-1, 3), K)
                projected_corners_gt = proj_pts.reshape(-1, 8, 2)
                draw_projection_boxes(canvas, projected_corners_gt, color=(0, 255, 0))   
...
class AppState:
    def __init__(self, data_loader):
        ...
        self.draw_pcd = not args.no_pcd
        # 移除固定的內外參儲存，因為現在改成每幀從 DataLoader 獲取
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LiDAR 視覺化工具")
    parser.add_argument("--base_folder", type=str, required=True, help="資料集路徑")
    ...
    parser.add_argument("--fps", type=int, default=10, help="幀率")
    parser.add_argument("--no_pcd", action="store_true", help="不繪製點雲 (只顯示框)")
    ...
    if not args.render_2d_video and not args.render_video:
